...
- [ ] Phần A – Test thủ công bằng Postman (đủ hàng / hết hàng) (Cần khởi động Docker MySQL & 2 service)
- [x] Phần B – Hiểu vì sao Integration Test cũ (Part 1) bị fail
- [x] Phần B – Thêm dependency WireMock (`spring-cloud-starter-contract-stub-runner`)
- [x] Phần B – Tạo `InventoryStubs` class
- [x] Phần B – Trỏ `inventory.url` sang WireMock trong test properties
- [x] Phần B – Viết lại `OrderServiceApplicationTests`
- [ ] Chạy `mvn test` PASS (Cần Docker daemon để chạy MySQL Testcontainer)
